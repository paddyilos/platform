...
| `src/Ushahidi/Core/Tool/Mailer.php` (`sendResetpassword()`) | Added `site_email` to the `$data` array passed to the view; changed the `from()` call from conditional (`if ($site_email) { $message->from($site_email, $site_name); }`) to unconditional, always using `$site_name` as the sender display name and falling back to `config('mail.from.address')` only for the address half | De-branding the password-reset email (see `forgot-password.php` below) needed the site's contact email available in the view, and the sender display name needed to reliably be the deployment name rather than silently falling back to the `MAIL_FROM_NAME` env var (which can drift out of sync with Settings → General's "Name" field) whenever `site_email` happens to be unset. |
