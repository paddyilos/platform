...
| `src/Ushahidi/Modules/V5/Http/Controllers/AnalysisTemplateController.php` | CRUD for saved Analysis dashboard report templates. Enforces `Permission::ACCESS_ANALYSIS` itself via `service('authorizer.post')->acl->hasPermission()` (defense in depth — same pattern `PostController` uses for permission checks; reuses the existing `authorizer.post` binding rather than registering a new `authorizer.*` binding since `Acl::hasPermission()` is generic). Validation rules extended (see `20260702000001_liberia_analysis_templates_multi_chart.php` below) to accept a `report_config` array (one `{group_by, group_by_attribute_key, chart_type}` per chart in a multi-chart report) plus template-wide `status_filter`/`tags_filter` arrays, replacing the single flat `group_by`/`chart_type`/dead `filters` shape. |
| `src/Ushahidi/Modules/V5/Models/AnalysisTemplate.php` | `analysis_templates` table model. `$fillable`/`$casts` extended for `report_config`/`status_filter`/`tags_filter` (see below); the original `group_by`/`group_by_attribute_key`/`chart_type` columns are kept as an unused fast-path summary of `report_config[0]`, for backward compatibility. |
...
| `database/migrations/phinx/20260702000001_liberia_analysis_templates_multi_chart.php` | Extends `analysis_templates` for multi-chart reports and template-wide filters: renames the previously-dead `filters` column (declared but never populated by any frontend code) to `report_config`, repurposed to hold a JSON array of per-chart configs (the ngx-charts equivalent of the old platform's "+ Add more charts"); adds `status_filter`/`tags_filter` JSON columns. Backfills any pre-existing single-chart row into a one-element `report_config` array so it still renders correctly under the new model. See `ushahidi-client/LIBERIA_CUSTOM.md`'s "Analysis / Analysis Templates" section for the frontend shape. |
| `migration/migrate-liberia-analysis-templates.php` | One-off, manually-run script (same PDO/dual-connection/`log_step()` conventions as `migrate-liberia.php`, but a separate script since the main migration already ran) that best-effort ports the ~7 real saved templates from the old `config` table (group `filters`, Flexmonster pivot-config JSON) into the new `analysis_templates` schema. Decomposes each old 2D pivot (`rows` × `columns`) into up to two single-dimension charts rather than fabricating a 2D crosstab equivalent; drops (and logs) anything with no clean mapping — a multi-level pivot, or a dimension name matching neither the special-case list (Province/District/Zone/Status) nor a migrated `form_attributes.label`. Must run after the `20260702000001` migration above. |
